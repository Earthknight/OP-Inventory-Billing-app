...
Future<List<Billing>> getbillingdata() async {
  // const jsonEndpoint =
  //     "http://192.168.0.7/products_php_files/getbillingdata.php";
  const jsonEndpoint =
      "http://192.168.0.105:80/php_workspace/inventory_app/getbillingdata.php";
  final response = await http.get(Uri.parse(jsonEndpoint));
  if (response.statusCode == 200) {
    List billing = await json.decode(response.body);
    return billing
        .map((billing) => Billing.fromJson(billing))
...
class _PayementCardState extends State<PayementCard> {
  ...
  Future<void> deletedata() async {
    // var url = Uri.parse(
    //     "http://192.168.0.7/products_php_files/deleteselectedproducts.php");
    var url = Uri.parse(
        "http://192.168.0.105:80/php_workspace/inventory_app/deleteselectedproducts.php");
  ...
  Future<void> updatePro() async {
    // var url =
    //     Uri.parse("http://192.168.0.7/products_php_files/removequanity.php");
    var url = Uri.parse(
        "http://192.168.0.105:80/php_workspace/inventory_app/removequanity.php");
  ...
  Future<void> insertData(List<Billing> l) async {
    ...
    var url = Uri.parse(
        "http://192.168.0.105:80/php_workspace/inventory_app/insertbilling.php");
    var response = await http.post(url, body: {
      "billingid": "B" +
          BillingIdnew(l)
              .toString(), //insertdata function in database refer inserbilling.php file
